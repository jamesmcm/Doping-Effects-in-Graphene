C$$$ WANT TO LOOP OVER DIFFERENT ENERGIES AND PRODUCE T^2 COEFFICIENTS, CHECK THEY MATCH WITH ANALYTICAL RESULTS
      ...
      DOUBLE PRECISION BEENAKKER
c$$   NB LIMX CHANGED TO 2       
      INTEGER, PARAMETER :: LIMX=500, WRAPY=0, WRAPX=1,
     + MSIZE=4*LIMX*LIMX, M2SIZE=LIMX*LIMX
      INTEGER I/1/, K/1/, F/1/, LIMY/866/
      CHARACTER*3 VALUE
      
      DOUBLE PRECISION RVALS(LIMX),  TVALS(LIMX),
     +                 TTVALS(LIMX), RTVALS(LIMX)

      DOUBLE PRECISION LAMBDA(LIMX)

      DOUBLE PRECISION COND/-1.0/
      DOUBLE PRECISION E/0.0/
      DOUBLE PRECISION G/1.0/
...
c$$$  CALCULATES THE LAMBDA VALUES GIVEN BY EQUATION IN BEENAKKER PAPER
      DO I = 1, LIMX
         LAMBDA(I) = BEENAKKER(TVALS(I))
         WRITE (*,40) I, LAMBDA(I)
      END DO
...
C$$$ SO T^2 + R^2 =1 FOR SVD VALUES, ALSO VERIFIED WITH R~ AND T~
 ...
 40   FORMAT (I4, 15ES15.5E3)
 50   FORMAT (F8.5,15ES15.5E3)      
 60   FORMAT (15ES15.5E3)
 70   FORMAT (15ES15.5E3, 15ES15.5E3)
...
c$$$ Needs a better name perhaps?  Sets to 0.0 if input is too close
c$$$ to 0.
      DOUBLE PRECISION FUNCTION BEENAKKER(TVAL)

      DOUBLE PRECISION TVAL
      DOUBLE COMPLEX INVERSECOSH

      IF (TVAL .GT. 1.0E-15) THEN
         BEENAKKER = REAL(INVERSECOSH(1/TVAL))
      ELSE
         BEENAKKER = 0.0
      END IF
...
c$$$ ACOSH(X) function doesn't seem to work
      DOUBLE COMPLEX FUNCTION INVERSECOSH(INPUT)

      DOUBLE PRECISION INPUT

      INVERSECOSH = LOG(INPUT+(SQRT(INPUT+1)*SQRT(INPUT-1)))
