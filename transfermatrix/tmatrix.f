...
c$$$  AIM TO CHANGE TO FILL(U,LIMX,FLUX)
      ...
      SUBROUTINE CALCMULTXY(NSIZE, MODD, MEVEN, E, FLUX)
C     NOTE LIMY MUST BE PASSED AS CONSTANT PARAMETER FOR THIS
      IMPLICIT NONE
      INTEGER LIMX
C     WRAPY ignored
      INTEGER I/1/, NSIZE
      ...
      DOUBLE COMPLEX MODD(NSIZE*2, NSIZE*2), MEVEN(NSIZE*2, NSIZE*2), 
     +     N3EVEN(NSIZE, NSIZE), N2EVEN(NSIZE, NSIZE), 
     +     N3ODD(NSIZE, NSIZE), N2ODD(NSIZE, NSIZE)

C     Odd is defined for odd leftmost column, even for even leftmost column
      CALL SQZERO (N3ODD, NSIZE)
      CALL SQZERO (N2ODD, NSIZE)
      CALL SQZERO (N3EVEN, NSIZE)
      CALL SQZERO (N2EVEN, NSIZE)
...
C     M={{-N3^-1, E*N3^-1},{-E*N3^-1, E^2 N3^-1 - N2^-1}}
      ...
      CALL INVERTMATRIX(N3ODD, NSIZE)
      CALL INVERTMATRIX(N2ODD, NSIZE)

      MODD(1:NSIZE, 1:NSIZE)=-1*N3ODD
      MODD(1:NSIZE, NSIZE+1:2*NSIZE)=E*N3ODD
      MODD(NSIZE+1:2*NSIZE, 1:NSIZE)=-E*N3ODD
      MODD(NSIZE+1:2*NSIZE, NSIZE+1:2*NSIZE)=(E*E*N3ODD) - N2ODD

      CALL INVERTMATRIX(N3EVEN, NSIZE)
      CALL INVERTMATRIX(N2EVEN, NSIZE)

      MEVEN(1:NSIZE, 1:NSIZE)=-1*N3EVEN
      MEVEN(1:NSIZE, NSIZE+1:2*NSIZE)=E*N3EVEN
      MEVEN(NSIZE+1:2*NSIZE, 1:NSIZE)=-E*N3EVEN
      MEVEN(NSIZE+1:2*NSIZE, NSIZE+1:2*NSIZE)=(E*E*N3EVEN) - N2EVEN
c$$$      CALL ZPRINTM(MODD, 2*NSIZE, "MO ")
